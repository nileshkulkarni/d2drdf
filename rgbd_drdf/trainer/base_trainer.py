        ...
        for epoch in range(start_epoch, opts.TRAIN.NUM_EPOCHS):
            ...
            for i in range(len(self.dataloader)):
                if True:
                    ...
                    batch_timer.toc()
                    t = batch_timer.get_time()
                    # logger.debug(f"dataloader per batch: {t}")
                ...
                if True:
                    ...
                    batch_timer.toc()
                    t = batch_timer.get_time()
                    # logger.debug(f"set input per batch: {t}")

                if not self.invalid_batch:
                    self.real_iter += 1
                    if True:
                        if self.real_iter > 5 and False:
                            with profile(
                    ...
                    if total_steps % opts.LOGGING.SAVE_CHECKPOINT_FREQ == 0:
                        ...
                        self.save_network(self.model, "model", "epoch_latest")
                        self.save_network(self.optimizer, "optimizer", "epoch_latest")
                        logger.info(
            ...
            if epoch % opts.LOGGING.SAVE_EPOCH_FREQ == 0:
                ## save frequency. Save stuff.
            ...
            if epoch % opts.LOGGING.VALID_EPOCH_FREQ == 0 and opts.TRAIN.VALIDATE:
                ## validation loss
